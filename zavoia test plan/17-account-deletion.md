...
> **QA 2026-07-25 addendum (found during the full QA-account wipe):** **BUG: owner deletion 500s (ACCOUNT.E01, clean rollback) when ANY customer has favorited one of the business's locations, and customer deletion 500s when the customer favorited any location** — `customer_favorite_location` carries NO-ACTION FKs to both `user` and `location` but appears in neither purge path (this is 17.16's "other customers' traces" case). Legacy `review` (customerId/employeeId) is the same class of hole on the user side. Fixed in admin-api 2026-07-25 (also added notification_event + staff_availability_day cleanup), pending deploy; live workaround: remove the favorite via API first. Also verified live: `deleteBusinesslessOwnerAccount` (d36340b) cleanly deletes owner-without-business accounts (the old 17.13 E02 dead-end is gone), and orphaned reminder Cloud Tasks are SAFE post-deletion (processReminder → appointment_not_found → markEventDone + ACK; nothing sent from the snapshot).

> **QA 2026-07-24 (API, throwaways +64/+66):** **17.1/17.4 CRITICAL fix verified live (7ced520): customer WITH bookings deletes cleanly** — {userDeleted:true}, token replay 401, business_customer row purged, re-login E38. 17.5 (+50 PLUS-active → E09, nothing deleted) ✓, 17.9 (capacitor → E10 + instructions email) ✓, 17.6/17.7/17.17 full trial-owner purge via throwaway +66 (register → wizard → S03 businessDeleted+userDeleted → token dead → email reusable) ✓, 17.13/17.14 guards ✓ (customer token → 401; owner-no-business → E02). TM-deletion group (17.8/17.10–12) still blocked: minting a disposable TM needs an emailed invite token (allowlist).
