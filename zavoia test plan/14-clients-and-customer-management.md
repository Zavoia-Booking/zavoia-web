...
> **QA 2026-07-24 (API):** 14.1/14.3–14.6/14.8/14.9/14.11/14.13–14.17/14.20 PASS. **Deployed fixes verified live (7ced520): validation (1-char name/501-char notes now 400) and merge-from-either-side (marketplace record survived in 3 fresh pairs incl. manual-id merges).** NEW BUG: after a merge, re-adding a manual client with that email → 409 E02 forever (add-manually dup check doesn't skip `merged` rows; fixed locally 2026-07-24, pending deploy). FINDING: duplicate detection fires ONLY at marketplace-row creation (customer's first booking) — a manual client added after the customer already booked is never auto-flagged (14.6's "detected at NEXT booking" only holds pre-first-booking; merge endpoint still works if called). /business-customers/list body is `{filters:[]}` and returns a bare array.
