...
> **QA 2026-07-24 (API+web):** 13.1–13.7/13.10/13.11/13.13–13.16/13.18–13.21 PASS with the QA Beauty RO listing (Bucharest) — bilingual/diacritics-free tag search ✓, city typo ✓, `NO_MATCH_FOR_TAGS`/`NO_AVAILABILITY_ON_DATE`/`RECOMMENDED`/`NO_RESULTS` fallbacks all observed, favorites unlisted-drop/restore ✓, SEO pages canonical+hreflang (rendered camelCased `hrefLang` — valid) ✓, /en 308 keeps query ✓. **Business 2 (Rădăuți) no longer exists on staging — single-listing environment again.** Note: no owner-facing "unpublish" endpoint exists (publish only sets isListed=true); location `isPublic` is the owner visibility lever — 13.11 wording should reflect that. `/team-member-account/marketplace-visibility` is DASHBOARD_USER-only by design (active TMs → 403; 13.16 needs an orphaned dashboard_user fixture).
