...
> **QA 2026-07-24 (API, +52's DE location as foreign tenant):** 16.1–16.15 PASS. **16.3 cross-tenant fix verified live (7ced520): foreign location → zeroed "Unknown Location".** July counts/revenue recomputed exactly (8 appts / 63000 minor incl-cancelled-count excl-cancelled-revenue ✓); isCurrentlyOpen flips with location-tz close-time edit ✓; open247 ✓; TM variant (staff null, own-scoped counts, my-reviews widget, selector = assigned locs) ✓. Shape notes: appointmentWidget = {today,week,month,upcoming} (no "distribution" key); reviewWidget counts BUSINESS reviews only (3.3/3 — /review/stats overall combines professional 3.5/6); needsAttention API carries ONLY unresolved_appointments — pending/SMS rows are composed client-side (NeedsAttentionWidget.tsx). FINDING (16.15): TM deep-link to an unassigned same-business location returns its name/open state (low). 16.16/16.17 still blocked — past-due appts not API-seedable.
