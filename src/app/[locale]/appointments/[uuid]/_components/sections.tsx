...
// ── ONLINE — dark card (joinOnline OMITTED as a non-functional URL: see note) ─

export function OnlineCard({ t, appt }: { t: ApptDict; appt: AppointmentDetail }) {
  const loc = appt.location;
  return (
    <div
      style={{
    ...
    >
      <div
        ...
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(60% 130% at 100% 0%, color-mix(in oklch, var(--p-500) 26%, transparent), transparent 60%)",
        }}
      />
      <div
        style={{
      ...
      >
        ...
        >
          <Icon name="globe" size={17} color="#fff" />
        </span>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 15.5, fontWeight: 600, letterSpacing: "-0.015em" }}>
            {loc?.name ?? t.onlineSessionFallback}
          </div>
          {loc?.address && (
            <div
              style={{
      ...
      {/* `joinOnline` button OMITTED: the API exposes no join URL, so a button
          would be non-functional. Documented in the manifest. */}
    </div>
  );
}
